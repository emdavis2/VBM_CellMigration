...
#function that updates the direction of the internal polarity bias direction based on gradient
...
def update_pol_dir(w_prev, y_curr, N, dt, sigma_w=1):
  xpos = y_curr[0:N] #x positions of all vertices at current time
  xpos_w = xpos[w_prev] #x position of vertex where polarity bias directionis pointing
  k_w = 5/lin_grad(xpos_w)
...
def EulerSolver(func, t_start, t_end, dt, y0, force_on_ind0, magnitude, pol_dir0, num_nearest_neighbors0, N, l0, A_0, params):
  ...
  for i in range(number_steps):
    ...
    pol_dir = update_pol_dir(pol_dir_prev, y_curr, N, dt, sigma_w)
